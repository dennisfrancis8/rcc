<?php 
    ...
    $active = JUMUIYASTATUS['ACTIVATED'];
    ...
    $query =  "SELECT * FROM `jumuiya` WHERE `jumuiyaStatus` = $active ORDER BY `jumuiya`.`jumuiyaId` ASC";
    ...
    if ($result->num_rows > 0) {
        ...
        for($i = 0; $i < $result->num_rows; $i++) {
            ...
            $newJumuiyaId = $row['jumuiyaId'];
            if ($newJumuiyaId == "INDIVIDUAL" || $newJumuiyaId == "WHOLE PARISH") {
                # do nothing...
            }
            else {
                $jumuiya = new Jumuiya();
                $jumuiya->jumuiyaId = $newJumuiyaId;
